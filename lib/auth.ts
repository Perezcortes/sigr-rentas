...
export async function authenticateUser(email: string, password: string): Promise<User | null> {
  ...
  if (typeof access === "string") setAccess(access)
  if (typeof refresh === "string") setRefresh(refresh)

  const meRaw = await req<any>("/auth/profile")
  const user = pickNormalizedUser(meRaw, loginBody, access ?? getAccess())
  return user ?? null
}
...
export async function fetchProfile(): Promise<User | null> {
  try {
    const meRaw = await req<any>("/auth/profile")
    const user = pickNormalizedUser(meRaw, null, getAccess())
    return user ?? null
  } catch {
    return null
  }
